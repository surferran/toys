...
void Bayang_send_packet(u8 bind)
{
    ...
    if (bind) {
		...
		chanval.value = map(ppm[AILERON], PPM_MIN, PPM_MAX, 0x3ff,0);   // aileron
        //chanval.value	= map(ppm[AILERON], PPM_MIN, PPM_MAX, 0, 0x3ff);   // aileron. original line
        ...
        ///packet[8]		= chanval.bytes.msb + DYNTRIM(chanval.value);;
        packet[9] = chanval.bytes.lsb;
		chanval.value = map(ppm[RUDDER], PPM_MIN, PPM_MAX, 0x3ff, 0);   // rudder
        //chanval.value	= map(ppm[RUDDER], PPM_MIN, PPM_MAX, 0, 0x3ff);   // rudder. original line
