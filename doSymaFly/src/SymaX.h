...
void Symax_init()
{
 ...
 ///   NRF24L01_Initialize();
    ...
    status_ans = NRF24L01_ReadReg(NRF24L01_07_STATUS);
  Serial.print("pcb status 1 : ");   Serial.println(status_ans);
    ...
    NRF24L01_WriteReg(NRF24L01_04_SETUP_RETR, 0xee); //ff  // 4mS retransmit t/o, 15 tries (retries w/o AA?)
    ...
    status_ans = NRF24L01_ReadReg(NRF24L01_07_STATUS);  //RAN addition
  Serial.print("pcb status 2 : ");   Serial.println(status_ans);
    NRF24L01_FlushTx();
...
static void SYMAX_build_packet(u8 bind) {
    if (bind) {
        ...
        packet[0] = map(ppm[THROTTLE]	, PPM_MIN, PPM_MAX, 0x00, 0xFF); 
   ...
   //         packet[3] = map(ppm[AILERON], PPM_MID, PPM_MAX, 0x80, 0xFF);
		/// my change
		packet[1] = map(ppm[ELEVATOR]	, PPM_MIN, PPM_MAX, 0x00, 0xFF);
		packet[2] = map(ppm[RUDDER]		, PPM_MIN, PPM_MAX, 0x00, 0xFF);
		packet[3] = map(ppm[AILERON]	, PPM_MIN, PPM_MAX, 0x00, 0xFF);
		///
        packet[4] = 0;
